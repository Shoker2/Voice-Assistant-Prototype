...
def lang_swith(text):
	...
	for i in lang_config['langs']:
		if command_name == lang_config['langs'].get(i):
			...
			global configs
			config['lang'] = i
			configs['config'] = config 		# дальше я просто перезаписываю нужные данные
...
try:
	...
	commands = [
	[lang_config['commands']['temperature'], lang_config['commands']['sign in'], lang_config['commands']['sign out'], lang_config['commands']['help'], lang_config['commands']['mixer'], lang_config['commands']['code'], lang_config['commands']['pH'], lang_config['commands']['lang'], lang_config['commands']['calibration']],
	[set_temperature, sing_in, sing_out, help_commands, mixer, code, pH, lang_swith, pH_calibration]
						...
						if authentication == True: # проверяю авторизирован пользователь или нет
							...
							if response == None:
								...
								if request.ok:
									...
									if request.text != 'Connection error':
